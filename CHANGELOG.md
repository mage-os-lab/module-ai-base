...
- Streamed tool calls arrive whole, with their arguments already accumulated and JSON-decoded by the provider bridge, so consumers no longer hand-parse SSE frames or stitch `input_json_delta` fragments. Reasoning ("thinking") deltas are surfaced as their own chunk type rather than mixed into the answer text.
